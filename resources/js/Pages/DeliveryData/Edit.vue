...
const form = useForm({
    sign: props.deliveryData.sign,
    address: props.deliveryData.address,
    city: props.deliveryData.city,
    province_id: props.deliveryData.province_id,
    region_id: props.deliveryData.region_id,
    note: props.deliveryData.note,
    reference_name: props.deliveryData.reference_name,
    telephone: props.deliveryData.telephone,
    email: props.deliveryData.email,
});

const submit = () => {
    form.put(route('dashboard.clients.delivery_data.update', [props.client, props.deliveryData]));
    console.log(form)
};
</script>

<template>
    <Head title="Edit delivery data" />
